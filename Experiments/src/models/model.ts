...
import { Regularizer } from '@tensorflow/tfjs-layers/dist/regularizers.js';
import { getLossArray, Loss } from './losses.js';
import { getMetricsObject, Metric } from './metrics.js';
...
export class Stats {
  constructor(
    ...
    public globalLoss: number[] = [],
    public globalMetric: number[] = [],
  ) {}
}
...
export class MTLModel {
  ...
  updateStats(
    ...
    testGlobalLosses?: Array<number>,
    testGlobalMetrics?: Array<number>
  ) {
    this.heads.forEach((head, i) => {
      ...
      // global evaluation
      if (testGlobalLosses !== undefined && testGlobalMetrics !== undefined) {
        if (head.stats.loss[0][0] === undefined) head.stats.globalLoss = []; // reset if first entry was undefined
        if (head.stats.metric[0][0] === undefined) head.stats.globalMetric = []; // reset if first entry was undefined
        head.stats.globalLoss.push(testGlobalLosses[i]);
        head.stats.globalMetric.push(testGlobalMetrics[i]);
      } 
  ...
  evaluate(xs: tf.Tensor4D, y: tf.Tensor[], batchSize: number) {
    ...
    for (let i = 0; i < index; i++) outputs[i].dispose(); // prevent leaks
    ...
    return { totalTestLoss, testLosses, testMetrics };
  }
}